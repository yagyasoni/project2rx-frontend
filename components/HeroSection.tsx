      ...
      <div className="container mx-auto px-6 pt-32 pb-16 relative z-10">
        <div className="text-center max-w-4xl mx-auto">
          ...
          >
            ...
            <span className="text-white/80 text-sm">Trusted by 300+ pharmacies</span>
          </motion.div>
        </div>
        ...
        >
          <div className="bg-card bg-white rounded-xl shadow-2xl overflow-hidden border border-border">
            ...
            <div className="bg-background">
              ...
              {/* Table rows */}
              {[...Array(12)].map((_, i) => (
                <div key={i} className="grid grid-cols-10 gap-2 px-4 py-3 text-sm border-b border-border/50">
                  {[...Array(10)].map((_, j) => (
                    <div key={j} className="h-3 bg-muted rounded w-full" style={{ width: `${60 + Math.random() * 40}%` }} />
                  ))}
                </div>
              ))}
            </div>
