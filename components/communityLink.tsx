...
export default function CommunityLinkPageCopy({
  ...
  const sortedData = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filteredData = filteredData.filter((row) => {
        ...
        const matchesMainRow =
          row.bin?.toLowerCase().includes(query) ||
          row.pcn?.toLowerCase().includes(query);

        // ✅ Search inside loaded groups
        const matchesGroup =
          groupDataMap[key]?.some((g) =>
            g.grp?.toLowerCase().includes(query),
          ) || false;

        return matchesMainRow || matchesGroup;
      });
    }
  ...
  }, [communityData, sortConfig, searchTerm]);
