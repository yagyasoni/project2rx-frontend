...
// // ✅ CHANGED: Empty by default — will be populated from DB
...
//   const { allSuppliers, selectedSuppliers, saveSuppliers, loading } = useSuppliers();
...
//       const fromDB = selectedSuppliers.map((name) => ({
//         id: Math.random().toString(36).substr(2, 9),
//         name,
//         file: null,
//       }));
//       setWholesalers(fromDB);
//     }
//   }, [selectedSuppliers, loading]);

//   const alreadyAddedNames = wholesalers.map((w) => w.name);
//   const availableForDrawer = allSuppliers.filter(
//     (name) => !alreadyAddedNames.includes(name)
//   );
...
//         metadata.push({ field: fieldName, wholesaler_name: w.name, headerMapping: wholesalerFieldMappings[w.id] || {} });
...
//     try {
//       const res = await axios.post(`https://api.auditprorx.com/api/audits/${id}/wholesalers`, formData);
...
//       const message = typeof data === "string" ? data : typeof data?.message === "string" ? data.message : typeof data?.error === "string" ? data.error : err?.message || "Please try again.";
...
//     // Auto-save newly added suppliers to DB + Settings
//     const newNames = selectedInDrawer.filter(
//       (name) => !selectedSuppliers.includes(name)
//     );
//     if (newNames.length > 0) {
//       const merged = [...selectedSuppliers, ...newNames];
...
//                 <p className="text-xs text-gray-500 mt-0.5">{uploadedCount} of {wholesalers.length} files uploaded</p>
//               </div>
//             </div>
//           </div>
...
//                 <div className="h-full bg-gray-900 rounded-full transition-all duration-500" style={{ width: `${(uploadedCount / wholesalers.length) * 100}%` }} />
//               </div>
//             </div>
//           )}
//         </div>
...
//           {/* ✅ Show empty state if no suppliers selected */}
//           {!loading && wholesalers.length === 0 && (
//             <div className="flex flex-col items-center justify-center py-12 text-center">
//               <Building2 className="w-8 h-8 text-gray-200 mb-3" />
//               <p className="text-sm font-medium text-gray-500">No suppliers added yet</p>
//               <p className="text-xs text-gray-400 mt-1">Click "Add Supplier" below to get started</p>
//             </div>
//           )}
...
//                       <button onClick={() => setShowMappingFor(wholesaler.id)} className="text-[11px] font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-100 px-2.5 py-1 rounded-lg transition-colors">Edit Mapping</button>
//                     )}
//                     <label htmlFor={`wholesaler-${wholesaler.id}`} className="cursor-pointer">
//                       <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${wholesaler.file ? "bg-white border border-emerald-200 text-emerald-700 hover:bg-emerald-50" : "bg-gray-900 text-white hover:bg-gray-800"}`}>
//                         <Upload className="w-3 h-3" />{wholesaler.file ? "Replace" : "Upload"}
//                       </span>
//                       <input id={`wholesaler-${wholesaler.id}`} type="file" className="hidden" onChange={(e) => handleFileChange(wholesaler.id, e)} accept=".csv" />
//                     </label>
//                     <div className="relative">
//                       <button className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors" onClick={() => { setEdit(!edit || editId !== wholesaler.id); setEditId(wholesaler.id); }}>
...
//                           <button onClick={() => handleDelete(wholesaler.id)} className="w-full px-3 py-2 text-left text-xs font-medium text-red-600 hover:bg-red-50 transition-colors">Delete</button>
//                         </div>
//                       )}
//                     </div>
//                   </div>
...
//                         <span className="text-[10px] text-emerald-600 font-medium bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full">Auto-detected ✓</span>
//                       </div>
...
//                         return (
//                           <div key={field.key} className={`flex items-center gap-4 px-4 py-2.5 transition-colors ${isMappedField ? "bg-white" : isRequired ? "bg-red-50/20" : "bg-white"}`}>
...
//                               <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${isRequired ? "text-red-500 bg-red-50 border-red-100" : "text-slate-400 bg-slate-50 border-slate-100"}`}>{isRequired ? "REQ" : "OPT"}</span>
//                             </div>
//                             <div className="relative w-48 flex-shrink-0">
//                               <select
//                                 className={`w-full border rounded-lg px-2.5 py-1.5 text-xs bg-white focus:outline-none focus:ring-2 transition-all ${isMappedField ? "border-emerald-300 text-emerald-800 focus:ring-emerald-200" : isRequired ? "border-red-300 text-gray-500 focus:ring-red-200" : "border-gray-200 text-gray-500 focus:ring-gray-200"}`}
//                                 value={currentMapping}
//                                 onChange={(e) => setWholesalerFieldMappings((prev) => ({ ...prev, [wholesaler.id]: { ...(prev[wholesaler.id] || {}), [field.key]: e.target.value } }))}
//                               >
//                                 <option value="">— Select column —</option>
//                                 {wholesalerHeaders[wholesaler.id]?.map((header) => (<option key={header} value={header}>{header}</option>))}
//                               </select>
//                             </div>
//                           </div>
//                         );
...
//                       <button onClick={() => setShowMappingFor(null)} className="flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-800 text-white text-xs font-semibold rounded-lg transition-colors">
//                         <CheckCircle2 className="w-3.5 h-3.5" />Confirm Mapping
//                       </button>
//                     </div>
//                   </div>
//                 )}
...
//           <button onClick={() => setIsDrawerOpen(true)} className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 rounded-xl transition-colors shadow-sm">
//             <Plus className="w-3.5 h-3.5" />Add Supplier
//           </button>
//           <div className="flex items-center gap-2">
//             <Link href="/ReportsPage">
//               <button className="px-4 py-2 text-xs font-semibold text-gray-600 bg-white border border-gray-300 hover:bg-gray-50 rounded-xl transition-colors">Skip</button>
//             </Link>
//             <button onClick={handleSubmit} className="flex items-center gap-2 px-5 py-2 bg-gray-900 hover:bg-gray-800 text-white text-xs font-semibold rounded-xl transition-colors shadow-sm">
...
//                 <p className="text-[11px] text-gray-500 mt-0.5">Select suppliers to add</p>
//                 {selectedInDrawer.length > 0 && (<p className="text-xs text-blue-600 mt-0.5">{selectedInDrawer.length} selected</p>)}
//               </div>
//               <button onClick={() => setIsDrawerOpen(false)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-200 transition-colors"><X className="w-4 h-4" /></button>
//             </div>
//             <div className="px-4 py-3 border-b border-gray-100">
//               <input type="text" placeholder="Search suppliers..." value={drawerSearch} onChange={(e) => setDrawerSearch(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-gray-300 bg-gray-50" />
//             </div>
...
//                   return (
//                     <div key={name} onClick={() => setSelectedInDrawer((prev) => prev.includes(name) ? prev.filter((i) => i !== name) : [...prev, name])}
//                       className={`flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-colors ${isSelected ? "bg-gray-900 text-white" : "hover:bg-gray-50 border border-transparent hover:border-gray-200"}`}>
...
//                   <p className="text-xs text-gray-400">{allSuppliers.length === 0 ? "No suppliers available." : "All suppliers have been added already."}</p>
//                 </div>
//               )}
//             </div>
//             <div className="px-4 py-4 border-t border-gray-200 bg-gray-50 flex gap-2">
//               <button onClick={() => setIsDrawerOpen(false)} className="flex-1 px-3 py-2 text-xs font-semibold text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">Cancel</button>
//               <button onClick={handleConfirmAddFromDrawer} disabled={selectedInDrawer.length === 0} className="flex-1 px-3 py-2 text-xs font-semibold text-white bg-gray-900 hover:bg-gray-800 rounded-xl transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
...
//               <div className="flex items-center gap-3">
//                 <div className="w-9 h-9 rounded-xl bg-red-100 flex items-center justify-center"><AlertTriangle className="w-5 h-5 text-red-600" /></div>
//                 <h3 className="text-base font-bold text-gray-900">Missing Required Columns</h3>
//               </div>
//               <button className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors" onClick={() => setShowMappingWarning(false)}><X className="w-4 h-4" /></button>
//             </div>
//             <p className="text-sm text-gray-600 mb-3">The following columns are required but not yet mapped:</p>
//             <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-4">
//               <div className="flex flex-wrap gap-1.5">
//                 {missingMappingFields.map((h) => (<span key={h} className="text-[11px] font-semibold text-red-700 bg-white border border-red-200 rounded-md px-2 py-1">{h}</span>))}
//               </div>
//             </div>
//             <p className="text-xs text-gray-500 mb-4">Please map all required columns before proceeding.</p>
//             <div className="flex justify-end">
//               <button onClick={() => setShowMappingWarning(false)} className="px-5 py-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold rounded-xl transition-colors">Got it</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//             <div className="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center mb-5"><CheckCircle2 className="w-8 h-8 text-emerald-500" /></div>
//             <h3 className="text-lg font-bold text-gray-900 mb-1">Files Uploaded Successfully</h3>
//             <p className="text-sm text-gray-500 mb-6">{wholesalers.filter((w) => w.file).length} wholesaler file{wholesalers.filter((w) => w.file).length !== 1 ? "s" : ""} have been uploaded and processed.</p>
//             <button onClick={() => { setUploadSuccess(false); onViewAudit(); }} className="px-6 py-2.5 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold rounded-xl transition-colors">OK</button>
//           </div>
//         </div>
//       )}
...
//             <p className="text-sm text-gray-500 mb-5">{wholesalers.filter((w) => w.file).length} file{wholesalers.filter((w) => w.file).length !== 1 ? "s" : ""} uploading</p>
//             <div className="w-full space-y-2">
//               <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
//                 <div className="h-2 rounded-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all duration-300 ease-out" style={{ width: `${uploadProgress}%` }} />
//               </div>
//               <div className="flex items-center justify-between">
//                 <span className="text-xs text-gray-400">{uploadProgress < 90 ? "Uploading files..." : uploadProgress < 100 ? "Processing on server..." : "Finalizing..."}</span>
...
// ✅ Maps DB standard field names → frontend field keys
const STANDARD_TO_FRONTEND_KEY: Record<string, string> = {
...
const UploadWholesalersStep = ({
  ...
  const { allSuppliers, selectedSuppliers, saveSuppliers, loading } =
    useSuppliers();
  ...
  // ✅ NEW: Convert admin's DB mapping to frontend mapping format
  // Admin stores: { "NDC_Number": "ndc_number", "Qty": "quantity" }
  //   meaning: CSV column "NDC_Number" → standard field "ndc_number"
  // Frontend needs: { ndcNumber: "NDC_Number", quantity: "Qty" }
  //   meaning: frontend key "ndcNumber" → actual CSV header "NDC_Number"
  const convertAdminMapping = (
    adminMapping: Record<string, string>,
    parsedHeaders: string[],
  ): Record<string, string> => {
    const frontendMapping: Record<string, string> = {};

    // For each admin mapping entry: csvHeader → standardField
    for (const [adminCsvHeader, standardField] of Object.entries(
      adminMapping,
    )) {
      const frontendKey = STANDARD_TO_FRONTEND_KEY[standardField];
      if (!frontendKey) continue;

      // Find the actual CSV header in parsed headers (case-insensitive match)
      const matchedHeader = parsedHeaders.find(
        (h) => h.toLowerCase().trim() === adminCsvHeader.toLowerCase().trim(),
      );

      if (matchedHeader) {
        frontendMapping[frontendKey] = matchedHeader;
      }
    }
  ...
  // ✅ UPDATED: handleFileChange now fetches admin mapping from DB
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          if (res.data?.mappings) {
            // Admin mapping found — convert and apply it
            const adminMapping = res.data.mappings;
            const converted = convertAdminMapping(adminMapping, parsedHeaders);

            console.log(
              `✅ Admin mapping found for ${wholesaler.name}:`,
              adminMapping,
            );
          ...
          console.log(
            `ℹ️ No admin mapping for ${wholesaler.name}, using auto-detect`,
          );
        }

        // ✅ Step 2: Fallback — use local auto-detect (alias matching)
        const auto = buildWholesalerAutoMapping(parsedHeaders);
        setWholesalerFieldMappings((prev) => ({ ...prev, [id]: auto }));
        setShowMappingFor(id);
      };
          ...
          {!loading && wholesalers.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Click "Add Supplier" below to get started
              </p>
            </div>
          )}
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                ...
                                <span
                                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${isRequired ? "text-red-500 bg-red-50 border-red-100" : "text-slate-400 bg-slate-50 border-slate-100"}`}
                                >
                                  {isRequired ? "REQ" : "OPT"}
                                </span>
                              </div>
                ...
                <div className="text-center py-8">
                  <p className="text-xs text-gray-400">
                    {allSuppliers.length === 0
                      ...
                      : "All suppliers have been added already."}
                  </p>
                </div>
              )}
            </div>
