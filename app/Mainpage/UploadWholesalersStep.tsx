...
const UploadWholesalersStep = ({
  ...
  const convertAdminMapping = (
    ...
    for (const [adminCsvHeader, standardField] of Object.entries(adminMapping)) {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          if (res.data?.mappings) {
            ...
            console.log(`✅ Admin mapping found for ${wholesaler.name}:`, adminMapping);
            console.log(`✅ Converted to frontend format:`, converted);

            if (Object.keys(converted).length > 0) {
              setWholesalerFieldMappings((prev) => ({ ...prev, [id]: converted }));
              setShowMappingFor(id);
              return; // Done — skip auto-detect
            }
          }
        } catch (err) {
          console.log(`ℹ️ No admin mapping for ${wholesaler.name}, using auto-detect`);
        }
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-2 relative">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-100 max-h-[420px] overflow-y-auto">
          {loading && wholesalers.length === 0 && (
            <div className="flex items-center justify-center py-12 text-xs text-gray-400">Loading your suppliers...</div>
          )}
          {!loading && wholesalers.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <Building2 className="w-8 h-8 text-gray-200 mb-3" />
