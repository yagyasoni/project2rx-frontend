...
//     await axios.post(`https://51.21.167.65/api/audits/${id}/inventory`, formData);
        ...
        <div className="border-t border-amber-200/60 bg-white/80 px-5 py-5 space-y-5">
          ...
          <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 flex items-start gap-2.5">
            ...
            <p className="text-[11px] text-amber-700 leading-relaxed">
              ...
              <span className="font-semibold">Submit Upload</span>.
            </p>
          </div>
        </div>
      )}
...
const UploadInventoryStep = ({
  ...
  const [fieldMapping, setFieldMapping] = useState<Record<string, string>>({});
  const [showWarning, setShowWarning] = useState(false);
  const [missingRequiredHeaders, setMissingRequiredHeaders] = useState<
    string[]
  >([]);
  const [submitSuccess, setSubmitSuccess] = useState(false);
              ...
              <div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
                {[...REQUIRED_FIELDS].map((field, idx) => {
                  ...
                  return (
                    ...
                    >
                      {/* Index */}
                      <span className="w-5 text-[10px] font-bold text-gray-300 flex-shrink-0">
                        {idx + 1}
                      </span>

                      {/* Status dot */}
                      <div
                        className={`w-2 h-2 rounded-full flex-shrink-0 ${isMapped ? "bg-emerald-400" : "bg-red-300"}`}
                      />
