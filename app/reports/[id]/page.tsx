...
const auditId = params.id as string
  ...
  highestShortage: 0, // will be calculated below
...
// Calculate rank by total billed descending (rank 1 = highest total billed)
const sortedByBilled = [...normalized].sort((a, b) => b.totalBilled - a.totalBilled);
sortedByBilled.forEach((row, index) => {
  row.rank = index + 1;
});

// Calculate highestShortage = most negative shortage value across all PBM columns
normalized.forEach((row: any) => {
  ...
  // Find the most negative value (biggest shortage)
                ...
                >
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      className="gap-2 border-slate-300 -translate-x-3"
                    >
                      QTY
                      <ChevronDown className="h-3.5 w-3.5" />
                    </Button>
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    ...
                    onCheckedChange={() => {
                      setQtyType("UNIT")
                      setColumnFilters(prev => ({ ...prev, unit: true, pkgSize: false }))
                    }}
                  >
                    UNIT
                  </DropdownMenuCheckboxItem>
                  ...
                  <TableRow className="hover:bg-transparent">
                    ...
                    {columnFilters.totalOrdered && (
                      <TableHead
                        ...
                        style={{
                          left: `${56 + (columnFilters.rank ? 80 : 0) + (columnFilters.ndc ? 140 : 0) + (columnFilters.drugName ? 240 : 0) + (columnFilters.pkgSize ? 100 : 0) + (columnFilters.unit ? 100 : 0)}px`,
                        }}
                      >
                        <HeaderCell
