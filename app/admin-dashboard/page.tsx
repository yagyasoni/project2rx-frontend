...
function SortHeader({
  ...
  return (
    <th
      className={`px-4 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider ${className}`}
    >
      <button
...
export default function AdminDashboard() {
  ...
  const handleSort = (key: typeof sortKey) => {
    ...
    setPage(1); // jump back to first page on sort change
  };
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                ...
                <div className="flex items-center gap-3 lg:ml-auto">
                  <span className="text-[11px] text-muted-foreground whitespace-nowrap">
                    {filtered.length - 1 === users.length - 1
                      ? filtered.length - 1
                      : filtered.length}{" "}
                    / {users.length - 1} shown
                  </span>
                  {hasActiveFilters && (
                    <Button
                      variant="ghost"
