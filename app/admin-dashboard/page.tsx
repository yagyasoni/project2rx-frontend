...
// import {
//   Users, RefreshCw, Search, ChevronRight, Phone, Mail, Calendar,
//   ShieldCheck, LogIn, Building2, Loader2, FileSpreadsheet, X, Plus,
//   Trash2, Save, AlertTriangle, CheckCircle2, Activity, TrendingUp,
//   Clock, PlusSquare, Database,
// } from "lucide-react";
...
//   color: "#374151", padding: "7px 12px", borderRadius: 8,
...
//   background: "#f1f5f9", border: "1px solid #e2e8f0",
//   color: "#374151", padding: "2px 7px", borderRadius: 4,
//   fontSize: 10, fontFamily: "'Poppins', sans-serif",
// };

// const COL_CONFIG: Record<string, { label: string; width: number; readOnly?: boolean }> = {
...
//       color: "#f8fafc", padding: "12px 18px", borderRadius: 12,
//       fontSize: 13, fontWeight: 500,
//       fontFamily: "'Poppins', sans-serif",
//       boxShadow: "0 8px 32px rgba(0,0,0,0.35)",
//       animation: "slideInRight 0.25s ease",
//     }}>
//       {type === "success" ? <CheckCircle2 size={16} color="#4ade80" /> : <AlertTriangle size={16} color="#f87171" />}
//       {message}
//       <button onClick={onClose} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", marginLeft: 4, display: "flex" }}>
//         <X size={14} />
//       </button>
//     </div>
//   );
...
// // ─────────────────────────────────────────────────────────────
// // EXCEL EDITOR MODAL — white background, black borders
// // ─────────────────────────────────────────────────────────────
// function ExcelEditorModal({ onClose, onToast }: {
//   onClose: () => void;
...
//     setLoading(true); setFetchError(""); setPage(0);
//     setDirtyRows(new Set()); setDeletedIds([]); setActiveCell(null);
//     try {
//       const res = await axios.get<ExcelState>(`${API_BASE}/admin/excel`);
//       setData(res.data);
//     } catch (err: any) {
//       setFetchError(err?.response?.data?.error || "Cannot reach API — make sure backend is running on port 5000.");
//       setData({ sheetName: "master_sheet", headers: ["id","bin","pcn","grp","pbm_name","payer_type"], rows: [], total: 0 });
...
//     return String(row[colIdx] ?? "").toLowerCase().includes(searchQuery.toLowerCase());
//   }) ?? [];

//   const totalPages = Math.max(1, Math.ceil(filteredRows.length / PAGE_SIZE));
//   const pageRows = filteredRows.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);

//   useEffect(() => { setPage(0); }, [searchQuery, searchCol]);

//   const activateCell = useCallback((globalVisIdx: number, ci: number, val: CellValue) => {
...
//   const commitEdit = useCallback((val: string, visIdx: number, ci: number) => {
//     if (!data) return;
...
//     setDirtyRows((prev) => { const s = new Set(prev); s.delete(realIdx); return s; });
//   };
...
//         await Promise.allSettled(deletedIds.map((id) => axios.delete(`${API_BASE}/admin/excel/row/${id}`)));
//       }
//       const res = await axios.post<{ message: string; updated: number; inserted: number }>(
...
//       onClose();
//     } catch (err: any) {
//       onToast(err?.response?.data?.error || "Save failed. Please try again.", "error");
...
//       <div
//         tabIndex={-1}
//         onKeyDown={(e) => { if (e.key === "Escape") { activeCell ? setActiveCell(null) : onClose(); } }}
//         style={{
//           width: "min(98vw, 1240px)", height: "min(94vh, 820px)",
//           background: "#ffffff",
...
//         {/* ── Modal Header ── */}
//         <div style={{
//           padding: "14px 20px", borderBottom: "1.5px solid #000000",
//           background: "#f8fafc",
...
//               background: "#111111", border: "1px solid #000000", flexShrink: 0,
//               display: "flex", alignItems: "center", justifyContent: "center",
//             }}>
//               <Database size={16} color="#ffffff" />
//             </div>
//             <div>
//               <div style={{ fontWeight: 700, fontSize: 13, color: "#111111", fontFamily: "'Poppins', sans-serif" }}>
//                 master_sheet
//               </div>
//               <div style={{ fontSize: 11, color: "#6b7280", display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
//                 {loading ? (
//                   <><Loader2 size={10} style={{ animation: "spin 1s linear infinite" }} color="#111111" /> Querying PostgreSQL…</>
//                 ) : (
//                   <><span style={{ color: "#111111" }}>●</span> {data?.rows.length ?? 0} rows · {data?.headers.length ?? 0} columns</>
//                 )}
//                 {fetchError && <span style={{ color: "#dc2626", marginLeft: 6 }}>{fetchError}</span>}
//               </div>
//             </div>
//           </div>

//           {/* Search bar */}
//           <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1, minWidth: 260, maxWidth: 480 }}>
//             <select
//               value={searchCol}
//               onChange={(e) => setSearchCol(e.target.value)}
//               style={{
//                 background: "#ffffff", border: "1px solid #000000", borderRadius: 8,
//                 color: "#374151", fontSize: 11, padding: "7px 8px",
//                 fontFamily: "'Poppins', sans-serif", outline: "none", cursor: "pointer",
//               }}
//             >
//               {(data?.headers ?? []).filter((h) => h !== ID_COL).map((h) => (
//                 <option key={h} value={h}>{COL_CONFIG[h]?.label ?? h}</option>
//               ))}
//             </select>
...
//                   borderRadius: 8, padding: "7px 28px 7px 28px", color: "#111111",
...
//               />
//               {searchQuery && (
//                 <button onClick={() => setSearchQuery("")} style={{ position: "absolute", right: 7, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#9ca3af", display: "flex" }}>
//                   <X size={12} />
//                 </button>
//               )}
//             </div>
//             {searchQuery && (
//               <span style={{ fontSize: 11, color: "#6b7280", fontFamily: "'Poppins', sans-serif", whiteSpace: "nowrap" }}>
...
//               <span style={{ fontSize: 11, color: "#d97706", fontFamily: "'Poppins', sans-serif" }}>
//                 {dirtyRows.size > 0 && `${dirtyRows.size} edited`}
//                 {dirtyRows.size > 0 && deletedIds.length > 0 && " · "}
//                 {deletedIds.length > 0 && `${deletedIds.length} deleted`}
//               </span>
//             )}
//             <button onClick={fetchSheet} style={toolbarBtn} title="Reload from DB"><RefreshCw size={13} /></button>
//             <button onClick={addRow} style={toolbarBtn}><PlusSquare size={13} /> Add Row</button>
//             <button
//               onClick={handleSave} disabled={saving}
//               style={{
//                 ...toolbarBtn,
//                 background: saving ? "#e5e7eb" : "#111111",
//                 border: "1px solid #000000",
//                 color: saving ? "#9ca3af" : "#ffffff",
//                 padding: "7px 18px",
//               }}
//             >
//               {saving ? <><Loader2 size={13} style={{ animation: "spin 1s linear infinite" }} /> Saving…</> : <><Save size={13} /> Save to DB</>}
//             </button>
//             <button onClick={onClose} style={{ ...toolbarBtn, padding: "7px 10px" }}><X size={15} /></button>
//           </div>
//         </div>
...
//             <Loader2 size={30} color="#111111" style={{ animation: "spin 1s linear infinite" }} />
...
//             <table style={{ borderCollapse: "collapse", fontSize: 12, fontFamily: "'Poppins', sans-serif", width: "100%" }}>
//               <thead style={{ position: "sticky", top: 0, zIndex: 20 }}>
//                 <tr>
//                   <th style={{ ...thBase, width: 54, textAlign: "center", background: "#e5e7eb", borderRight: "1px solid #000000" }}>#</th>
//                   {data?.headers.map((h, ci) => {
//                     const cfg = COL_CONFIG[h] ?? { label: h, width: 140 };
//                     return (
//                       <th key={ci} style={{ ...thBase, minWidth: cfg.width, width: cfg.width }}>
//                         <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
//                           {cfg.readOnly && (
//                             <span style={{ fontSize: 8, color: "#ffffff", background: "#374151", border: "1px solid #000000", borderRadius: 3, padding: "1px 5px" }}>PK</span>
//                           )}
//                           <span style={{ color: cfg.readOnly ? "#6b7280" : "#111111" }}>{cfg.label}</span>
//                         </div>
...
//                     );
//                   })}
//                   <th style={{ ...thBase, width: 44, textAlign: "center" }}><Trash2 size={11} color="#374151" /></th>
//                 </tr>
//               </thead>
//               <tbody>
//                 {pageRows.length === 0 ? (
//                   <tr>
//                     <td colSpan={(data?.headers.length ?? 0) + 2} style={{ textAlign: "center", padding: "64px 0", color: "#9ca3af", fontSize: 13, fontFamily: "'Poppins', sans-serif" }}>
//                       {searchQuery ? `No rows match "${searchQuery}"` : "master_sheet is empty — add a row or check your backend."}
//                     </td>
//                   </tr>
//                 ) : (
...
//                       <tr
//                         key={visIdx}
//                         style={{ background: visIdx % 2 === 0 ? "#ffffff" : "#f9fafb" }}
//                         onMouseEnter={(e) => (e.currentTarget.style.background = "#f0f4ff")}
//                         onMouseLeave={(e) => (e.currentTarget.style.background = visIdx % 2 === 0 ? "#ffffff" : "#f9fafb")}
//                       >
//                         {/* Row number */}
//                         <td style={{
//                           ...tdBase, textAlign: "center", color: "#6b7280", fontSize: 10,
//                           fontWeight: 600, background: isDirty ? "#fef9c3" : "#f1f5f9",
//                           borderRight: "1px solid #000000",
//                         }}>
//                           {isDirty && <span style={{ color: "#d97706", marginRight: 2 }}>●</span>}
...
//                         {row.map((cell, ci) => {
//                           const header = data!.headers[ci];
//                           const cfg = COL_CONFIG[header] ?? { label: header, width: 140 };
//                           const isActive = activeCell?.r === globalVisIdx && activeCell?.c === ci;
//                           const isId = !!cfg.readOnly;
//                           return (
//                             <td
//                               key={ci}
//                               onClick={() => !isId && activateCell(globalVisIdx, ci, cell)}
//                               style={{
//                                 ...tdBase, width: cfg.width, minWidth: cfg.width,
//                                 outline: isActive ? "2px solid #111111" : "none",
//                                 outlineOffset: -1,
...
//                                   autoFocus
//                                   value={editValue}
//                                   onChange={(e) => { setEditValue(e.target.value); commitEdit(e.target.value, globalVisIdx, ci); }}
//                                   onKeyDown={(e) => {
...
//                                       let nextCi = ci + 1;
//                                       while (nextCi < headers.length && COL_CONFIG[headers[nextCi]]?.readOnly) nextCi++;
//                                       if (nextCi < headers.length) activateCell(globalVisIdx, nextCi, pageRows[visIdx][nextCi]);
//                                       else setActiveCell(null);
//                                     }
//                                   }}
//                                   style={{
//                                     width: "100%", background: "transparent", border: "none",
//                                     outline: "none", color: "#111111", fontSize: 12,
//                                     fontFamily: "'Poppins', sans-serif", padding: 0,
//                                   }}
//                                 />
//                               ) : isId ? (
//                                 <span style={{ color: "#9ca3af", fontWeight: 600, fontSize: 11 }}>
//                                   {cell !== null && cell !== "" ? cell : <span style={{ color: "#d1d5db" }}>auto</span>}
//                                 </span>
//                               ) : (
//                                 <span style={{ color: cell === null || cell === "" ? "#9ca3af" : "#111111" }}>
...
//                         {/* Delete */}
//                         <td style={{ ...tdBase, textAlign: "center", padding: "8px 6px" }}>
//                           <button
//                             onClick={() => deleteRow(globalVisIdx)}
//                             style={{ background: "none", border: "none", color: "#d1d5db", cursor: "pointer", display: "flex", margin: "0 auto", padding: 4, borderRadius: 4, transition: "color 0.1s" }}
...
//                 {!searchQuery && (
//                   <tr>
//                     <td colSpan={(data?.headers.length ?? 0) + 2} style={{ padding: 0, borderTop: "1px solid #000000" }}>
//                       <button
//                         onClick={addRow}
//                         style={{
...
//                         }}
//                         onMouseEnter={(e) => { e.currentTarget.style.background = "#f9fafb"; e.currentTarget.style.color = "#111111"; }}
...
//         {/* ── Modal Footer ── */}
//         <div style={{
//           padding: "10px 20px", borderTop: "1.5px solid #000000", background: "#f8fafc",
//           display: "flex", alignItems: "center", justifyContent: "space-between",
//           flexShrink: 0, gap: 12, flexWrap: "wrap",
//         }}>
//           <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 11, color: "#6b7280" }}>
//             {[["Click","select"],["Tab","next col"],["Enter","confirm"],["Esc","deselect"]].map(([k, v]) => (
//               <span key={k} style={{ display: "flex", alignItems: "center", gap: 4 }}>
//                 <kbd style={kbdStyle}>{k}</kbd> {v}
//               </span>
//             ))}
//             <span style={{ color: "#9ca3af", marginLeft: 4 }}>ID = read-only PK</span>
//           </div>
//           <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
//             <span style={{ fontSize: 11, color: "#6b7280", fontFamily: "'Poppins', sans-serif" }}>
//               {searchQuery ? `${filteredRows.length} / ${data?.rows.length ?? 0}` : `${data?.rows.length ?? 0} rows`}
//             </span>
//             <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} style={{ ...toolbarBtn, padding: "5px 8px", opacity: page === 0 ? 0.25 : 1 }}>‹</button>
//             <span style={{ fontSize: 11, color: "#374151", fontFamily: "'Poppins', sans-serif", minWidth: 72, textAlign: "center" }}>
//               {page + 1} / {totalPages}
//             </span>
//             <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} style={{ ...toolbarBtn, padding: "5px 8px", opacity: page >= totalPages - 1 ? 0.25 : 1 }}>›</button>
//           </div>
//         </div>
//       </div>
//     </div>
...
//       </div>
//       <div>
//         <div style={{ fontSize: 22, fontWeight: 800, color: "#0f172a", lineHeight: 1, fontFamily: "'Poppins', sans-serif" }}>
//           {value}
//         </div>
//         <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 3, textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600 }}>
//           {label}
//         </div>
//       </div>
//     </div>
...
//   // Inject Poppins font + global keyframes
//   useEffect(() => {
...
//               color: "#64748b", padding: "7px 13px", borderRadius: 9,
//               fontSize: 12, cursor: "pointer", fontWeight: 500,
...
//             <div>
//               <h1 style={{ fontSize: 21, fontWeight: 800, color: "#0f172a", margin: 0, letterSpacing: "-0.5px", fontFamily: "'Poppins', sans-serif" }}>
//                 Pharmacy Users
//               </h1>
...
//               label="This Month"
//               value={users.filter((u) => { const d = u.createdAt ? new Date(u.createdAt) : null; return d && d.getMonth() === new Date().getMonth(); }).length}
//               accent="#10b981"
//             />
...
//             borderRadius: 18, overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,0.04)",
//           }}>
...
//                 </div>
//                 <span style={{ background: "#0f172a", color: "#f8fafc", fontSize: 10, fontWeight: 700, padding: "2px 10px", borderRadius: 20, fontFamily: "'Poppins', sans-serif" }}>
//                   {filtered.length}
//                 </span>
//               </div>
...
//                       <div style={{ fontWeight: 700, fontSize: 13, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "'Poppins', sans-serif" }}>
//                         {user.name}
//                       </div>
...
//                     display: "flex", alignItems: "center", justifyContent: "center",
//                     fontWeight: 900, fontSize: 22, color: "#f8fafc", fontFamily: "'Poppins', sans-serif",
//                   }}>
//                     {avatarChar(selected.name)}
//                   </div>
//                   <div style={{ flex: 1 }}>
//                     <div style={{ fontWeight: 800, fontSize: 17, color: "#f8fafc", letterSpacing: "-0.3px", fontFamily: "'Poppins', sans-serif" }}>{selected.name}</div>
//                     <div style={{ fontSize: 12, color: "#475569", marginTop: 4, display: "flex", alignItems: "center", gap: 6, fontFamily: "'Poppins', sans-serif" }}>
...
//                         </div>
//                         <div style={{ fontSize: 13, color: "#0f172a", fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "'Poppins', sans-serif" }}>
//                           {item.value}
//                         </div>
//                       </div>
//                     ))}
...
//                       background: impersonating ? "#e2e8f0" : "#0f172a",
//                       color: impersonating ? "#94a3b8" : "#f8fafc",
//                       fontSize: 13, fontWeight: 700,
//                       cursor: impersonating ? "not-allowed" : "pointer",
//                       display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
//                       boxShadow: impersonating ? "none" : "0 4px 20px rgba(15,23,42,0.2)",
...
//                       : <><LogIn size={15} /> Open as {selected.name}</>
//                     }
//                   </button>
//                   <p style={{ textAlign: "center", fontSize: 11, color: "#94a3b8", margin: "8px 0 0", fontFamily: "'Poppins', sans-serif" }}>
...
// ─────────────────────────────────────────────────────────────
const ADMIN_EMAIL = "admin@pharmsys.com";
// Reads from env on AWS, falls back to localhost for dev
const API_BASE = process.env.NEXT_PUBLIC_API_URL || "https://api.auditprorx.com";
// Max rows per POST request — keeps each payload ~30KB, well under any nginx limit
...
const thBase: React.CSSProperties = {
  ...
  textAlign: "left", color: "#111111", fontSize: 11,
  fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase",
  userSelect: "none", whiteSpace: "nowrap", fontFamily: "'Poppins', sans-serif",
};

const tdBase: React.CSSProperties = {
  padding: "8px 14px", borderBottom: "1px solid #000000",
  borderRight: "1px solid #000000", fontSize: 12,
  transition: "background 0.08s", whiteSpace: "nowrap",
  fontFamily: "'Poppins', sans-serif",
};
...
function Toast({ message, type, onClose }: {
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 8px 32px rgba(0,0,0,0.35)", animation: "slideInRight 0.25s ease",
    }}>
      {type === "success" ? <CheckCircle2 size={16} color="#4ade80" /> : <AlertTriangle size={16} color="#f87171" />}
      {message}
      <button onClick={onClose} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", marginLeft: 4, display: "flex" }}>
...
function ExcelEditorModal({ onClose, onToast }: {
  ...
  const fetchSheet = async () => {
    ...
    try {
      ...
      setFetchError(err?.response?.data?.error || "Cannot reach API.");
      setData({ sheetName: "master_sheet", headers: ["id","bin","pcn","grp","pbm_name","payer_type"], rows: [], total: 0 });
    } finally { setLoading(false); }
  };
  ...
  // ── Commit edit — FIX: use globalVisIdx into filteredRows ──
  // BUG THAT WAS HERE: visIdx (0–149 local page index) was passed
  // but filteredRows is the FULL list, so on page 2+ the wrong
  // row was being updated. Now we always pass globalVisIdx.
  const commitEdit = useCallback((val: string, globalVisIdx: number, ci: number) => {
    if (!data) return;
    // globalVisIdx is the index into filteredRows (not pageRows)
  ...
  // ── Chunked save — fixes 413 Payload Too Large ─────────────
  // Sends SAVE_CHUNK_SIZE (200) rows per request instead of all
  // 2462 rows at once. Each chunk is ~30KB, well under nginx limit.
  const handleSave = async () => {
    if (!data) return;
    setSaving(true);
    setSaveProgress(0);
    try {
      ...
      if (deletedIds.length > 0) {
        await Promise.allSettled(
          deletedIds.map((id) => axios.delete(`${API_BASE}/admin/excel/row/${id}`))
        );
      }

      // Step 2: send all rows in chunks of SAVE_CHUNK_SIZE
      ...
      for (let i = 0; i < totalChunks; i++) {
        const chunk = allRows.slice(i * SAVE_CHUNK_SIZE, (i + 1) * SAVE_CHUNK_SIZE);
        const res = await axios.post<{ message: string; updated: number; inserted: number }>(
          `${API_BASE}/admin/excel`,
          { sheetName: data.sheetName, headers: data.headers, rows: chunk }
        );
      ...
      onToast(`✓ Saved — ${totalUpdated} updated, ${totalInserted} inserted.`, "success");
      onClose();
    } catch (err: any) {
      onToast(err?.response?.data?.error || "Save failed. Please try again.", "error");
    } finally {
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          width: "min(98vw, 1240px)", height: "min(94vh, 820px)",
          background: "#ffffff", border: "1.5px solid #000000", borderRadius: 16,
          display: "flex", flexDirection: "column",
          boxShadow: "0 24px 80px rgba(0,0,0,0.18)",
          overflow: "hidden", outline: "none", fontFamily: "'Poppins', sans-serif",
        }}
      >
        {/* ── Header ── */}
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 36, height: 36, borderRadius: 9, background: "#111111",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginLeft: "auto" }}>
            ...
            <button
              ...
              style={{
                ...
                padding: "7px 18px",
                position: "relative", overflow: "hidden", minWidth: 145,
              }}
            >
              {/* Progress bar crawls behind the label */}
              {saving && (
                <div style={{
                  position: "absolute", left: 0, top: 0, bottom: 0,
                  width: `${saveProgress}%`, background: "rgba(74,222,128,0.25)",
              ...
              <span style={{ position: "relative", display: "flex", alignItems: "center", gap: 6 }}>
                {saving
                  ? <><Loader2 size={13} style={{ animation: "spin 1s linear infinite" }} />
                      {saveProgress > 0 && saveProgress < 100 ? `Saving ${saveProgress}%` : "Finishing…"}</>
          ...
          <div style={{ flex: 1, overflow: "auto" }}>
            <table style={{ borderCollapse: "collapse", fontSize: 12, fontFamily: "'Poppins', sans-serif", width: "100%" }}>
              ...
              <tbody>
                {pageRows.length === 0 ? (
                  <tr>
                    <td colSpan={(data?.headers.length ?? 0) + 2} style={{ textAlign: "center", padding: "64px 0", color: "#9ca3af", fontSize: 13 }}>
                      {searchQuery ? `No rows match "${searchQuery}"` : "master_sheet is empty."}
                    </td>
                  </tr>
                ) : (
                  pageRows.map((row, visIdx) => {
                    // globalVisIdx = position in filteredRows (not just this page)
                    // This is what commitEdit and deleteRow must receive
                    const globalVisIdx = page * PAGE_SIZE + visIdx;
                    const realIdx = data!.rows.indexOf(row);
                    const isDirty = dirtyRows.has(realIdx);
                              ...
                              {isActive ? (
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    // ✅ FIX: pass globalVisIdx (not visIdx)
                                    // visIdx is 0–149 (local page position)
                                    // globalVisIdx is the actual index in filteredRows
                                    // Without this fix, on page 2+ the wrong row gets updated
                                    commitEdit(e.target.value, globalVisIdx, ci);
                                  }}
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter") { e.preventDefault(); setActiveCell(null); }
                                    if (e.key === "Escape") setActiveCell(null);
                                    if (e.key === "Tab") {
                                      ...
                                      if (nextCi < headers.length) {
                                        activateCell(globalVisIdx, nextCi, pageRows[visIdx][nextCi]);
                                      } else {
                                        setActiveCell(null);
                                      }
                                    }
...
function StatCard({ icon, label, value, accent }: {
  ...
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 22, fontWeight: 800, color: "#0f172a", lineHeight: 1, fontFamily: "'Poppins', sans-serif" }}>{value}</div>
        <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 3, textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600 }}>{label}</div>
      </div>
    </div>
  );
}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button
            ...
            style={{
              ...
              cursor: "pointer", fontWeight: 600,
              boxShadow: "0 2px 12px rgba(22,163,74,0.25)",
              transition: "all 0.15s", letterSpacing: "0.02em", fontFamily: "'Poppins', sans-serif",
            }}
            onMouseEnter={(e) => (e.currentTarget.style.boxShadow = "0 4px 20px rgba(22,163,74,0.4)")}
            onMouseLeave={(e) => (e.currentTarget.style.boxShadow = "0 2px 12px rgba(22,163,74,0.25)")}
          >
      ...
      <div style={{ maxWidth: 1380, margin: "0 auto", padding: "24px 28px" }}>
        <div style={{ marginBottom: 22 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            ...
            <StatCard icon={<Activity size={18} color="#0ea5e9" />}  label="Active Users"   value={users.length} accent="#0ea5e9" />
            <StatCard
              icon={<TrendingUp size={18} color="#10b981" />} label="This Month"
              value={users.filter((u) => { const d = u.createdAt ? new Date(u.createdAt) : null; return d && d.getMonth() === new Date().getMonth(); }).length}
              accent="#10b981"
            />
            <StatCard icon={<Clock size={18} color="#f59e0b" />}    label="Pending Review" value={0}            accent="#f59e0b" />
                ...
                <div style={{ padding: "16px 26px", borderTop: "1px solid #f1f5f9", background: "#fafafa" }}>
                  <button
                    ...
                    style={{
                      ...
                      display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
                      boxShadow: impersonating ? "none" : "0 4px 20px rgba(15,23,42,0.2)",
                      transition: "all 0.15s", letterSpacing: "0.01em", fontFamily: "'Poppins', sans-serif",
                    }}
                    onMouseEnter={(e) => { if (!impersonating) e.currentTarget.style.background = "#1e293b"; }}
                    onMouseLeave={(e) => { if (!impersonating) e.currentTarget.style.background = "#0f172a"; }}
                  >
